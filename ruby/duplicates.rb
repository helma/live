...
@samples = Dir["#{File.join(ARGV[0],'*[wav|WAV]')}"].collect{|f| Sample.new(f,bpm)}
...
def dissimilarity_matrix
    ...
    @samples.each_with_index do |s,i|
      ...
      @samples[0..i-1].each_with_index do |s2,j|
        if s == s2
          sim = 1
        else
          sim = s.similarity s2
...
def duplicate_candidates threshold=0.05
...
puts duplicate_candidates.collect{|c| c.collect{|s| s.file}.join "\n"}.join "\n\n"
